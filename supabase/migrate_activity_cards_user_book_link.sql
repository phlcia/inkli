-- Link activity_cards to user_books for richer feed rendering

ALTER TABLE activity_cards
  ADD COLUMN IF NOT EXISTS user_book_id UUID REFERENCES user_books(id) ON DELETE CASCADE;
...
WITH matches AS (
  SELECT
    ...
    ROW_NUMBER() OVER (PARTITION BY ac.id ORDER BY ub.updated_at DESC) AS rn
  FROM activity_cards ac
  JOIN user_books ub
    ON ub.user_id = ac.user_id
  JOIN books b
    ON b.id = ub.book_id
  WHERE ac.user_book_id IS NULL
    AND (ub.updated_at = ac.created_at OR ub.created_at = ac.created_at)
    AND ac.content ILIKE '%' || b.title || '%'
)
UPDATE activity_cards ac
SET user_book_id = m.user_book_id
FROM matches m
WHERE ac.id = m.activity_id
  AND m.rn = 1;
